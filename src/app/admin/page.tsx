...
export default function AdminPage() {
  ...
  const tabs: { key: Tab; label: string; icon: any }[] = [
    { key: "pending", label: "În așteptare", icon: Clock },
    { key: "approved", label: "Aprobate", icon: CheckCircle2 },
    { key: "all", label: "Toate", icon: Users },
  ];
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-navy-deep/50">
          Gestionează cererile de aderare și membrii PFPSS
        </p>
      </div>
            ...
            >
              <div className="flex items-start justify-between gap-4 mb-4">
                ...
                <span className={`text-xs font-medium px-2.5 py-1 rounded-full border shrink-0 ${
                  ...
                  {org.status === "approved" ? "Aprobat" : org.status === "pending" ? "În așteptare" : "Respins"}
                </span>
              </div>

              <div className="grid sm:grid-cols-3 gap-4 py-4 border-t border-navy-deep/5">
                <div>
                  <p className="text-xs text-navy-deep/40 mb-1">Persoană contact</p>
                  <p className="text-sm font-medium text-navy-deep">{org.contact_name}</p>
                </div>
                <div>
                  <p className="text-xs text-navy-deep/40 mb-1">Email</p>
                  <p className="text-sm font-medium text-navy-deep">{org.contact_email}</p>
                </div>
                <div>
                  <p className="text-xs text-navy-deep/40 mb-1">Telefon</p>
