...
const categoryIcons: Record<string, any> = {
  ...
  "model": FileText,
  "raport": BarChart3,
  "regulament": FileCheck,
};

export default function DocumentePage() {
  const t = useTranslations("dashboard.documente");
  const [docs, setDocs] = useState<Doc[]>([]);
  const [loading, setLoading] = useState(true);
  const [approved, setApproved] = useState(false);

  const categoryConfig: Record<string, { label: string; icon: any }> = {
    "cod-etic": { label: t("categories.codEtic"), icon: Scale },
    "ghid": { label: t("categories.ghid"), icon: BookOpen },
    "model": { label: t("categories.model"), icon: FileText },
    "raport": { label: t("categories.raport"), icon: BarChart3 },
    "regulament": { label: t("categories.regulament"), icon: FileCheck },
  };
    ...
    <div className="max-w-4xl">
      ...
      {!approved ? (
        <div className="flex items-start gap-3 p-5 rounded-xl bg-amber-50 border border-amber-200">
          ...
          <div>
            <p className="font-medium text-amber-800">{t("restrictedTitle")}</p>
